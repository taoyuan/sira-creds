 ...
 * @param {Object} app The Ollo app instance
 * @returns {PassportConfigurator}
 * @constructor
 * @class
 */
function PassportConfigurator(app, sapp) {
    if (!(this instanceof PassportConfigurator)) {
        return new PassportConfigurator(app, sapp);
    }
    this.app = app;
    this.sapp = sapp;
}
...
PassportConfigurator.prototype.setupModels = function (options) {
    // Set up relations
    var self = this;
    self.userModel = self.sapp.model('User');
    self.userCredentialModel = self.sapp.model('UserCredential');
    self.userIdentityModel = self.sapp.model('UserIdentity');

    if (!self.userModel.relations.identities) {
        self.userModel.hasMany(self.userIdentityModel, {as: 'identities'});
    } else {
        self.userIdentityModel = self.userModel.relations.identities.modelTo;
    }

    if (!self.userModel.relations.credentials) {
        self.userModel.hasMany(self.userCredentialModel, {as: 'credentials'});
    } else {
        self.userCredentialModel = self.userModel.relations.credentials.modelTo;
    }
}
...
PassportConfigurator.prototype.configureProvider = function (name, options) {
    ...
    var AuthStrategy = require('sira-passport')[options.module][options.strategy || 'Strategy'];
